# 반복문 for
# for i in [1,2,3,4,5]:       # 묶음형 자료형 :list, tuple, set ...
# for i in (1,2,3,4,5):
for i in {1,2,3,4,5}:
    print(i, end = ' ')
...
print('\n정규 표현식 + for')
str = """청와대가 뉴미디어 분야를 대상으로 신규 출입기자 모집 공고를 냈다. 이재명 대통령은 정책에 대한 홍보가 중요하다고 강조해왔고, 투명성을 강조해온 이재명 정부는 청와대를 출입하는 유튜브 기반 매체를 늘리겠다고 한 바 있다.
...
for test_ss in ['111-1234', '일이삼-일이삼사', '222-1234','333&1234']:
    if re.match(r'^\d{3}-\d{4}$', test_ss):  # ^ : 시작, $ : 끝 ->숫자 3자리로 시작해서 4자리 숫자로 끝나야함
