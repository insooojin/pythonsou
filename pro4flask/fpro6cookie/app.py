# 문자열로 작성한 jinja템플릿을 텐더링 해 HTML로 변환
# redirect: 다른 url로 이동하기
# url_for: 라우트 함수 이름으로 url을 안전하게 생성하는 함수
# Cookie 는 브라우저에 저장되는 작은 키-값  데이터이고, 서버가 클라이언트와 연결 상태를 유지
# 하는 것처럼 할 수 있다. 
...
def delete_cookie():
    # 쿠키 삭제 후 홈(/)으로 이동하기 위한 redirect 응답을 만든다
